...
import { Question, QuizData, EditQuizFormProps } from '../../models/models';


const EditQuizForm: React.FC<EditQuizFormProps> = ({ quiz, onUpdateQuiz, onCancel }) => {
          ...
          {q.options.map((option, optionIndex) => (
            <div key={optionIndex}>
              ...
              <input
                type="text"
                value={option}
                onChange={(e) => handleQuestionChange(index, `option_${optionIndex}`, e.target.value)}
                className="p-2 border rounded w-full"
                required
              />
            </div>
